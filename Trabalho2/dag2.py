...
@dag(default_args=default_args, schedule_interval='@once', catchup=False, tags=["Guilherme", "PUC", "dag2"])
def trabalho2_dag2():
    ...
    def ind_media_passageiros(nome_do_arquivo):
        ...
        res.rename(
            columns={'total_passageiros': 'media_passageiros'}, inplace=True)
        res.rename(columns={'preco_medio': 'media_preco'}, inplace=True)
        res.rename(
            columns={'total_sibsp_parch': 'media_sibsp_parch'}, inplace=True)
        print(res)
